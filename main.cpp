...
public:
    ...
    // Add constructor here to initialize members
    League(std::ifstream &player_in, std::ifstream &gym_in) {
        player = Trainer_factory(player_in);
    ...
    ~League() {
        // put code here to clean up by deleting all Trainer objects
        delete player;
        for (int i = 0; i < gym_leaders.size(); ++i) {
...
int main(int argc, char *argv[]) {
    ...
    // Add code to read command line args and open file streams here
    ...
    // Create a League object, call function to run the simulation
    League l(player_in, gym_in);
    l.battle();
}